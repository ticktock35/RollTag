...
4. 一次超過 10 檔時，進度條有「**終止**」：停掉目前這次送出，已打的留下，還沒打的不再送。

大批次後面幾支可能因平台配額或逾時失敗，檔案本身往往沒問題。狀態列會寫成功與失敗支數。等幾分鐘再對還沒打的跑一次即可。
...
Settings: `~/rolltag/config.json` (do not share). Tags and thumbs: `<warehouse>/.rolltag/`. Full contract: [SPEC.md](SPEC.md).
